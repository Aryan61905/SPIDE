...
def getStats(conn,cursor,select_v,table_v,where_v,val,order="DESC",num = 50):
...
def getList(conn,cursor, select_v,table_v):
    cursor.execute(f"Select Distinct {select_v} from {table_v}")
    return [cursor.fetchall(),[description[0] for description in cursor.description]]


def getOrderedStats(conn,cursor,select_v,table_v,val,order="DESC",num = 5):
    cursor.execute(f"SELECT {select_v} FROM {table_v} ORDER BY {val} {order} LIMIT {num}")
    return cursor.fetchall()



def getBoxScoreStats(conn,cursor,select_v,table_v,where_v,val,limit,order="ASC"):
    order_val = False
...
def getBoxScoreStatsByDateRange(conn,cursor,select_v,table_v,where_v,val,sd,ed,order="ASC"):
