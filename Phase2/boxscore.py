...
df = pd.read_csv("/Users/roy/Desktop/SPIDE/Schedule.csv")
...
df['Token'] = df['Date'].apply(lambda x: f"{x[-4:]}{months[x.split(',')[1][1:4]]}{x.split(',')[1][5:7]}")+"0"+df["Home_Token"]



df = df[df['Date'].str.contains('Oct')]
...
for index, row in df.iterrows():
    ...
    path = f"/Users/roy/Desktop/SPIDE/BoxScores/{token}VS{visitor}.csv"
